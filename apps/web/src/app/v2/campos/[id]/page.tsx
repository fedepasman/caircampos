...
export async function generateMetadata({
  ...
  // reemplaza a la página actual — ver el comentario en robots.ts. Se deja
  // Open Graph/Twitter armados igual (no cuesta nada y sirve para probar
  // que el mecanismo funciona), pero apuntando a esta URL de `/v2`, no a la
  // real — y sin `alternates.canonical` hacia `/campos/[id]`, que sería
  // declarar esta página como duplicado de esa.
  return {
    title: `${campo.titulo} (draft)`,
...
export default async function FichaCampoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const jsonLd = {
    ...
    url: `${env.NEXT_PUBLIC_SITE_URL}/v2/campos/${id}`,
    image:
  ...
  return (
    <main>
      ...
      {/* Mismo hero de la sección "Campos" (no una foto genérica sin
          relación) — la foto real de este campo puntual sigue siendo la
          galería de abajo, este banner es la cabecera de la página, no un
          reemplazo de esa galería. */}
      <HeroSeccion
        src="/imagenes/hero-campos.jpg"
        alt=""
        titulo={campo.titulo}
        subtitulo={`${campo.localidad}, ${campo.provincia}`}
      />
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mt-10">
              ...
              <div className="mt-3 h-72 overflow-hidden rounded-lg border border-neutral-600 shadow-lg sm:h-96">
                <MapaCampos campos={[campo]} basePathFicha="/v2/campos" />
              </div>
            </div>
          </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-3">
              {otrosCampos.map((otro) => {
                const objectKey = [...otro.campo_fotos].sort((a, b) => a.orden - b.orden)[0]
                  ?.object_key;
                return (
                  <Link key={otro.id} href={`/v2/campos/${otro.id}`}>
